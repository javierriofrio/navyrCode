...
function writeNgcFilesToDisk(context, typescriptFilePath, typescriptFileContent, transpiledFileContent, sourcemapContent) {
    var dirName = path_1.dirname(typescriptFilePath);
    var relativePath = path_1.relative(process.cwd(), dirName);
    var tmpPath = path_1.join(context.tmpDir, relativePath);
    var fileName = path_1.basename(typescriptFilePath);
    var fileToWrite = path_1.join(tmpPath, fileName);
    var jsFileToWrite = helpers_1.changeExtension(fileToWrite, '.js');
    fs_extra_1.mkdirpSync(tmpPath);
    fs_extra_1.writeFileSync(fileToWrite, typescriptFileContent);
    fs_extra_1.writeFileSync(jsFileToWrite, transpiledFileContent);
    fs_extra_1.writeFileSync(jsFileToWrite + '.map', sourcemapContent);
}
